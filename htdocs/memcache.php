...
$fastPersistor = \Foomo\Cache\Manager::getPersistorFromConf($_SERVER['FOOMO_CACHE_FAST'], false);
if (empty($fastPersistor)) {
	die('fastPersistor is not configured');
}
if (is_a($fastPersistor, 'Foomo\Cache\Persistence\Fast\MemcachedPersistor') || is_a($fastPersistor, 'Foomo\Cache\Persistence\Fast\MemcachePersistor')) {
	$config = $fastPersistor->serverConfig;
} else {
	die('configured fastPersistor is not for memcached');
}
if (empty($config)) {
	die('configuration for memcached fastPersistor is empty or cannot be read');
}
$MEMCACHE_SERVERS[] = $config['host'] . ':' . $config['port']; // add more as an array
