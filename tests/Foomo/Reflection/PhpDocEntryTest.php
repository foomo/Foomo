...
class PhpDocEntryTest extends \PHPUnit_Framework_TestCase {
	const MOCK_NAMESPACE = 'Foomo\\Reflection';
	...
	public function testClassProps()
	{
		...
		$namespaces = array('', $classRefl->getNamespaceName());
		foreach($namespaces as $namespace) {
			$classDoc = new PhpDocEntry($classRefl->getDocComment(), $namespace);
	...
	{
		$namespaces = array('', self::MOCK_NAMESPACE);
		foreach($namespaces as $namespace) {
			$docEntry = $this->getTestMethodDocEntry($namespace);
	 ...
	 * @return PhpDocEntry
	 */
	private function getTestMethodDocEntry($namespace)
	{
		$methodRefl = new \ReflectionMethod(self::MOCK_CLASS_NAME, 'foo');
		return new PhpDocEntry($methodRefl->getDocComment(), $namespace);
	}
	...
	public function testClassProp()
	{
		...
		$namespaces = array('', self::MOCK_NAMESPACE);
		foreach($namespaces as $namespace) {
			$docEntry = new PhpDocEntry($refl->getDocComment(), $namespace);
	...
	{
		foreach(array('', __NAMESPACE__) as $namespace) {
			$entry = new PhpDocEntry(null, $namespace);
